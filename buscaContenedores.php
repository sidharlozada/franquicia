...
<? if($opcion=="1"){ ?>
   ...
   <div class="modal-header">
      <h5 class="modal-title" id="exampleModalLabel"><b>Contenedores</b></h5>
      ...
      <div class="row">
        <!-- <div class="col-sm-3">
            <div class="input-group ">
               <label>
                  <select class="inputForm" id="codigo2" name="codigo2" onchange="buscaPopup(<?=$pagina?>);">
...
<?}?>    
      <?php
         $regProd = new controlContenedor;
         $prod = $regProd->getContenedorAct($conn, $descripcion, $inicio, $tamano_pagina); 
         $total = $regProd->total; 
      ?>
                  ...
                  <tbody id="muestra">
                    <?php
                     if (is_array($prod) && count($prod)>0){
                        ...
                        {
                          ?>
                           <tr style="cursor: pointer;" data-dismiss="modal" aria-label="Close"  onclick="asignaRef('<?=$productos->container_control_id?>','<?=$productos->container_reference?>')">
                              <td><?=$productos->container_reference?></td>
                              <td><?=muestrafecha($productos->start_date)?></td>
                              <td><?=$productos->origen_desc.' - '.$productos->destino_desc?></td>
...
function buscaPopup(nro){
 ...
 // setTimeout(function(){
  ...
  $(muestra).load("buscaContenedores.php", {descripcion:$('#descripcion2').val(),codigo:$('#codigo2').val(),pagina:nro});
 // }, 1000);
